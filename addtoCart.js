...
function removeFromCart(productId) {
  ...
  if (removedItem) {
    toast(`${removedItem.name} removed from cart`, "warn");
  } else {
...
function updateCart() {
  ...
  cart.forEach((product) => {
    ...
    li.innerHTML = `${product.name} - $${product.price.toFixed(2)} x ${product.quantity} 
            <button class="removeButton" data-product-id="${
              product.id
            }">Remove</button>`;
...
function loadCart() {
  ...
  if (saved) {
    try {
      ...
      cart = parsed.filter(
        (item) =>
          item.id && !isNaN(item.price) && item.price > 0 && item.quantity > 0
      );
    } catch {
      cart = [];
...
function handleBuy() {
  ...
  if (!waitingForConfirm) {
    ...
    toast(
      `Your Total Is $${total.toFixed(2)} — Press BUY again to confirm`,
      "info",
      9999999999999999
    );

    return; // DO NOT reset automatically
  }

  // SECOND CLICK → Process purchase
  waitingForConfirm = false;

  toast("Processing your purchase...", "info", 2000);
...
function toast(message, type = "info", duration = 2500) {
  ...
  // Reset classes
  t.classList.remove("show", "success", "error", "info", "warn");

  // Force refresh for Android Chrome
  void t.offsetWidth;
